...
p_ortg_forecast <- autoplot(fc_ortg) +
    labs(
...
# Display plot
print(p_ortg_forecast)

# Save for conclusion
ggsave("images/ortg_forecast.png", plot = p_ortg_forecast, width = 12, height = 6, dpi = 300, bg = "white")
```
...
# Bridging to Multivariate Analysis

The univariate models above establish that NBA metrics and betting stocks exhibit predictable time-series structure. ORtg, 3PAr, and Pace all follow non-stationary trends that become forecastable after differencing, while DKNG and PENN stock prices display volatility clustering and seasonal patterns. These findings confirm that basketball's evolution and the betting industry's growth are not random walks but structured processes shaped by innovation, strategy, and market forces.

Yet univariate analysis has a fundamental limitation: **it treats each series in isolation**. When we model ORtg using only its own past values, we ignore the reality that offensive efficiency doesn't evolve independently. It's shaped by strategic decisions (3PAr), player skill (TS%), tempo (Pace), and defensive adaptation. Similarly, betting stock prices don't fluctuate in a vacuum—they respond to NBA attendance, game quality, regulatory changes, and broader market sentiment.

The next chapter shifts from **single-equation forecasting** to **system-level dynamics** using multivariate time series models. ARIMAX extends ARIMA by incorporating exogenous predictors, testing hypotheses like "Does 3PAr predict ORtg?" while controlling for autocorrelation. VAR (Vector Autoregression) goes further by modeling bidirectional feedback loops where every variable predicts every other variable, revealing whether the analytics revolution followed a simple cause-effect sequence or emerged from complex co-evolution.

**What you'll see next:**

- **ARIMAX models** quantify how shot selection and shooting skill predict offensive efficiency, testing whether analytics-driven strategy translates to measurable performance gains
- **VAR models** uncover temporal ordering through Granger causality tests, answering: Did 3PAr drive ORtg, or did efficient offenses enable more three-point volume?
- **COVID intervention analysis** isolates the pandemic's structural break from underlying trends using dummy variables
- **NBA-to-betting linkage** tests whether on-court analytics created financial value by modeling DKNG stock as a function of NBA attendance and offensive rating

Having established that each series is forecastable, we now ask: **Do they forecast each other?** The analytics revolution transformed basketball, but did it follow a single driver or emerge from interconnected forces? Multivariate models provide the framework to find out.
